...
class TacoApp{
    ...
    public function __construct($db){
        ...
        $this->rechercheArticle = $db->prepare("select top 9000 ta.[Numéro d'appareil] AS tacid, ta.Appareil AS appareil, tm.idTaco , od.PCyclage, od.[Désignation] AS desi, od.id, o.[Order Number] AS idCom, LEFT(o.[Order date],4) AS année, RIGHT(o.[Order date],2) AS jour, SUBSTRING(o.[Order date], 5, 2) AS mois, o.[Customer number] AS numClient, o.[Invoice number] AS idFacture from tacoApp ta, tacomaster tm, OrderDetail od, [Order] o where ta.[Numéro d'appareil] = tm.NbrApp and tm.[Plan de cyclage] = od.PCyclage and od.OrderNb = o.[Order number] and od.[Désignation] like :rechercheArticle order by od.[Désignation]");
