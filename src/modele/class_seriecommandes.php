...
class OrderSerial{
    ...
    public function __construct($db){
        ...
        //requete avec la vraie BD :
        $this->rechercheGarantie = $db->prepare("select top 9000 os.SrlNbr AS numSerie, o.[Order number] AS numCommande,LEFT(o.[Order date],4) AS année, RIGHT(o.[Order date],2) AS jour, SUBSTRING(o.[Order date], 5, 2) AS mois, o.[Customer number] AS numClient, i.InvoiceNumber AS numFacture, LEFT(i.[Invoice Date],4) AS annéeFact, RIGHT(i.[Invoice Date],2) AS jourFact, SUBSTRING(i.[Invoice Date], 5, 2) AS moisFact, i.[Invoice Name] AS nomClient, i.[Invoice Adress 1] AS adresse1, i.[Invoice Adress 2] AS adresse2, i.[Invoice Adress 3] AS adresse3, i.[Invoice ZIP] AS zip from OrderSerial os, [Order] o, Invoice i where os.InvoiceNbr=i.InvoiceNumber and os.OrderNbr=o.[Order number] and os.SrlNbr like :rechercheGarantie order by os.SrlNbr");
