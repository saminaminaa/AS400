...
class OrderDetail{
    ...
    public function __construct($db){
        ...
        $this->recherche = $db->prepare("select od.id, od.NbrArt, od.Désignation, od.Forklift, od.Manu, od.OrderNb AS numCommande, o.[Customer number] AS numClient, o.[Order number], o.[Order date] AS dateCommande, o.[Invoice number] AS numFacture, i.idInvoice, i.[Invoice Name] AS nomClient from OrderDetail od, [Order] o, Invoice i where od.OrderNb = o.[Order number] and i.OrderNumber = o.[Order number] and i.[Invoice Name] like :recherche order by od.Désignation");
        ...
        $this->rechercheChariot = $db->prepare("select od.Id, od.NbrArt, od.[Désignation] AS desi, od.Forklift, od.Manu, od.OrderNb, tm.idTaco AS idTm, tm.[Plan de cyclage] AS plancycl, ta.[Numéro d'appareil] AS idTa, ta.Constructeur AS constr, ta.Famille AS fam, ta.Appareil As app, tc.PlanCoffre AS pcof, tc.[Longueur 1] AS l1, tc.[Largeur 1] AS lar1, tc.[Hauteur 1] AS h1, tc.[Longueur 2] AS l2, tc.[Largeur 2] AS lar2, tc.[Hauteur 2] AS h2 from OrderDetail od, Tacomaster tm, TacoApp ta, TacoCof tc where od.Pcyclage = tm.[Plan de cyclage] and tm.NbrApp = ta.[Numéro d'appareil] and tm.[Plan de coffre] = tc.PlanCoffre and od.forklift like :rechercheChariot order by ta.Appareil");
        ...
        $this->selectById = $db->prepare("select top 1 NbrArt, [Désignation] AS desi, Forklift, Manu, InvNbr AS idFacture, ShpNbr AS idLivraison, OrderNb AS idCommande, PCyclage AS planCyclage, PlanChantier planChantier from OrderDetail od where id=:id");
