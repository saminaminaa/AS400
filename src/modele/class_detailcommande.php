...
class OrderDetail{
    ...
    public function __construct($db){
        ...
        $this->recherche = $db->prepare("select top 9000 od.id, od.NbrArt, od.Désignation, od.Forklift, od.Manu, od.OrderNb AS numCommande, o.[Customer number] AS numClient, o.[Order number], LEFT(o.[Order date],4) AS année, RIGHT(o.[Order date],2) AS jour, SUBSTRING(o.[Order date], 5, 2) AS mois, o.[Invoice number] AS numFacture, i.idInvoice, i.[Invoice Name] AS nomClient from OrderDetail od, [Order] o, Invoice i where od.OrderNb = o.[Order number] and i.OrderNumber = o.[Order number] and i.[Invoice Name] like :recherche order by od.Désignation");
